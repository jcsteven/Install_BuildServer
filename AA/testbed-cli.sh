...
function usage
{
  ...
  echo "                               ptf64  t0-64            t1-8vm      t1-64-lag"
  echo "                                      t0-64-32         t1-8vm-lag"
  echo "                                      t0-52"
  echo "                                      t0-116"
...
function add_topo
{
  ...
  # ignore the replace operation if user no specify vmbase
  if [ -n "$vmbase" ]; then
      awk_str="{ if (\$1 == topo) \$9=vm; print > \"$tbfile\";} "
      cat $tbfile | awk -v topo=$topology -v vm=$vmbase "$awk_str"  FS="," OFS=","
  fi
...
while getopts "t:m:k:n:b:s:d:" OPTION; do
    case $OPTION in
    t)
        tbfile=$OPTARG
        ;;
